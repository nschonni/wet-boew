...
(function ($, window, document, undef) {
    ...
    var getUrlParts = function (url) {
        ...
        return {
            ...
            params: (function () {
                var results = {}, queryString = encodeURI(decodeURI(a.search.replace(/^\?/, ""))).replace(/'/g, "%27").split("&"), len = queryString.length, key, strings, i;
                for (i = 0; i !== len; i += 1) {
     ...
     */
    lang = document.documentElement.lang, paths = (function (ele) {
        var paths = {};
    ...
    oldie = (function () {
        ...
        while ((div.innerHTML = "<!--[if gt IE " + (v += 1) + "]><i></i><![endif]-->",
            all[0])) { }
        ;
        return v > 4 ? v : undef;
    }()),
     ...
     */
    disabled = (function () {
        var disabledSaved = "false", disabled;
        try {
    ...
    wb = {
        ...
        init: function (event, componentName, selector, noAutoId) {
            var eventTarget = event.target, isEvent = !!eventTarget, node = isEvent ? eventTarget : event, initedClass = componentName + "-inited", isDocumentNode = node === document;
        ...
        add: function (selector) {
            var exists = false, len = wb.selectors.length, i;
        ...
        remove: function (selector) {
            var len = this.selectors.length, i;
            for (i = 0; i !== len; i += 1) {
        ...
        timerpoke: function (initial) {
            var selectorsLocal = wb.selectors.slice(0), len = selectorsLocal.length, selector, $elms, $foundElms, i;
            if (initial) {
                $foundElms = $();
                for (i = 0; i !== len; i += 1) {
    ...
    Modernizr.load([
        {
            ...
            complete: function () {
                var componentName = "wb-math", selector = "math", math = document.getElementsByTagName(selector), $document = wb.doc;
